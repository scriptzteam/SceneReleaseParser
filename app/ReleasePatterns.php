...
class ReleasePatterns
{
    ...
    const REGEX_TITLE_EBOOK =
        "/^" .
        self::REGEX_TITLE .
        "[._(-]+(?:%year%|%language%|%flags%|%format%|%regex_date%|%regex_date_monthname%)[._)-]/iU";

    const REGEX_TITLE_FONT = "/^" . self::REGEX_TITLE . "-/i";

    const REGEX_TITLE_MOVIE =
        "/^" .
        self::REGEX_TITLE .
        "[._(-]+(?:%year%|%language%|%source%|%flags%|%format%|%resolution%|%audio%)[._)-]/iU";
    const REGEX_TITLE_MOVIE_EXTRA =
        "/%year%[._-]" . self::REGEX_TITLE . "[._(-]\.+/iU";

    const REGEX_TITLE_MUSIC =
        "/^" .
        self::REGEX_TITLE .
        "(?:\([\w-]+\))?[._(-]+(?:\(?%source%\)?|%year%|%group%|%audio%|%flags%|%format%|%regex_date%|%regex_date_monthname%|%language%[._)-])/iU";
    const REGEX_TITLE_ABOOK =
        "/^" .
        self::REGEX_TITLE .
        "[._(-]+(?:%source%[._)-]|%year%|%group%|%audio%|%flags%|%format%|%language%[._)-])/iU";
    const REGEX_TITLE_MVID =
        "/^" .
        self::REGEX_TITLE .
        "[._(-]+(?:%source%|%year%|%group%|%audio%|%flags%|%format%|%regex_date%|%regex_date_monthname%|%language%[._)-])/iU";

    const REGEX_TITLE_APP =
        "/^" .
        self::REGEX_TITLE .
        "[._(-]+(" .
        self::REGEX_VERSION .
    ...
    const REGEX_TITLE_TV =
        "/^" . self::REGEX_TITLE . "[._-]" . self::REGEX_EPISODE_TV . "/iU";

    const REGEX_TITLE_TV_EPISODE =
        "/" .
        self::REGEX_EPISODE_TV .
        "[._-](?:" .
        self::REGEX_TITLE .
        "[._\(-]+)?\.+/iU";
    const REGEX_TITLE_TV_DATE =
        "/^" .
        self::REGEX_TITLE .
        "[._\(-]+(?:%regex_date%|%year%)[._\)-]" .
        self::REGEX_TITLE .
        "?[._\(-]?(?:%language%[._\)-]|%resolution%|%source%|%flags%|%format%)/iU";

    const REGEX_TITLE_XXX =
        "/^" .
        self::REGEX_TITLE .
        "[._\(-]+(?:%year%|%language%[._\)-]|%flags%)/iU";

    const REGEX_TITLE_XXX_DATE =
        "/^" .
        self::REGEX_TITLE .
        "[._\(-]+(?:%regex_date%|%regex_date_monthname%)[._\)-]+" .
        self::REGEX_TITLE .
        "[._\(-]+(?:%flags%|%language%[._\)-])/iU";

    const REGEX_VERSION_TEXT = "(?:v(?:ersione?)?|Updated?[._-]?v?|Build)";
    const REGEX_VERSION =
        self::REGEX_VERSION_TEXT . "[._-]?([\d.]+[a-z\d]{0,3}(?![._-]gage))";
    ...
    const OS = [
        ...
        "Windows" =>
            "win(?:(?:[\d]+[\dxk]?|nt|all|dows|xp|vista|[msp]e)?[._-]?){0,6}",
        "Windows CE" => "wince",
    ...
    const FLAGS = [
        ...
        "Convert" => "CONVERT",
        "Cover" =>
            "(?:(?:CUST?OM|%language%|%format%|%source%|%resolution%|scans?|disc|ps[\dp]*|xbox\d*|gamecube|gc|unrated|\d*DVD\w+|hig?h?.?res|int|front|retail|\d+dpi|r\d+|original)[._-]?)COVERS?",
        "CPOP" => "CPOP",
        "Incl. Crack" => [
            "CRACK.?ONLY",
            "(?:incl|working)[._-](?:[a-zA-Z]+[._-])?crack",
        ],
        "Cracked" => "CRACKED",
        "Crackfix" => "CRACK.?FIX",
        "Criterion" => "CRITERION",
        ...
        "Dubbed" => [
            "(?<!line.|line|mic.|mic|micro.|tv.)Dubbed",
            "E.?Dubbed",
            "(?!over|thunder)[a-z]+dub",
        ],
        "eBook" => "EBOOK",
        "Extended" => "EXTENDED(?:.CUT|.Edition)?(?!.MIX)",
        "Final" => "FINAL[._-]?(%language%|%flags%)?",
        "FiX" =>
            "(?<!hot.|sample.|nfo.|rar.|dir.|crack.|sound.|track.|diz.|menu.)FiX(?:.?only)?",
        ...
        "KEY" => "GENERIC.?KEY",
        "KEYGEN" => [
            "(?:Incl.)?KEY(?:GEN(?:ERATOR)?|MAKER)(?:.only)?",
            "KEYFILE.?MAKER",
        ],
        "Intel" => "INTEL",
        ...
        "Incl. Patch" => [
            "(?:incl.)?(?:[a-z]+[._-])?patch(?:ed)?(?:[._-]only)",
            "no[a-zA-Z]+[._-]patch(?:ed)?(?:[._-]only)",
        ],
        "Paysite" => "PAYSITE",
        "Portable" => "Portable",
        "Preair" => "PREAIR",
